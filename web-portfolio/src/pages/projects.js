...
const GisProjects = () => {
    ...
    return (
        <Layout>
            <div>
            <section className='min-h-screen grid bg-gray-light dark:bg-gray-dark p-10 xs:grid-cols-1 lg:grid-cols-2 gap-8 content-start justify-center md:h-screen'>
                    {data.Projects.map(project => (
                        <div key={`${project.name}1`} className='relative overflow-hidden'>
                            <div key={`${project.name}2`} className='relative aspect-w-16 aspect-h-9'>
                                <Image src={projectImage[project.thumbnail]} alt={project.name} className='h-auto max-w-full shadow-md opacity-100 hover:opacity-30 transition duration-300' priority />
                            </div>
